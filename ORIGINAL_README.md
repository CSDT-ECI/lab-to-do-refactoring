...
3. Si cambias propiedades de la base de datos como la contraseña o el usuario en el archivo `application.properties`, debes compilar el proyecto de nuevo con Maven. Para hacerlo, utiliza el comando:
   ...
   Podrás encontrar tu aplicación en http://localhost:8080/login.xhtml, deberás crear un usuario provisional registrando un usuario y dándole permisos de `Administrador` y estado de cuenta `Activo` con un cliente de base de datos como [DBeaver](https://dbeaver.io/)

5. Si desea empaquetar en un jar asegúrate de tener el `application.properties` configurado con la base de datos a la que te vas a conectar luego de eso ejecuta el comando:

    ```bash
    mvn clean package
    ```

6. Para ejecutar la aplicación usa el siguiente comando:
...
![Diagrama de Despliegue](assets/images/despliegue.png)
